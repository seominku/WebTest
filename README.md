...
[웹사이트 접속하기 · 원본 화면 미리보기](https://seominku.github.io/WebTest/)

> **원본 사이트의 디자인과 화면을 그대로 사용하는 정적 미리보기이며, 서비스를 운영하지 않습니다.** 홈·매물 목록·상세·로그인·회원가입 등 원본 화면을 둘러볼 수 있습니다. 가상 매물 3개만 사용하며 지도·사진·㎡/평 전환은 동작합니다. 로그인·가입·검색 조건 적용·등록·문의·업로드·주변 시설 조회는 비활성 상태입니다. GitHub Pages가 화면을 제공하므로 PC나 임시 터널에 의존하지 않습니다. 실제 개인정보를 입력하지 마세요.

배포 방식·검증·제한: [원본 화면 GitHub Pages 미리보기](./md/32_원본화면_GitHub_Pages_미리보기.md). `apps/web` 원본 소스를 별도 임시 폴더로 복사한 뒤 공개용 서버 차단 설정을 적용해 자동 배포합니다. 원본 앱의 인증·API·DB 기능은 삭제하거나 비활성화하지 않습니다.
