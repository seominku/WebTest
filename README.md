...
[웹사이트 접속하기 · GitHub Pages 데모](https://seominku.github.io/WebTest/)

> **로그인 없이 매물·지도·샘플 사진을 보는 읽기 전용 데모입니다.** GitHub Pages가 제공하므로 PC·Docker·임시 터널에 의존하지 않고, 기존 테스트 사이트의 밤 10시 13분 종료 제한도 적용되지 않습니다. 가상 매물 3개만 포함하며 로그인·등록·문의·업로드·실시간 DB 연동은 지원하지 않습니다. 기존 전체 앱의 운영 배포와는 별개입니다.

배포 방식·검증·제한: [GitHub Pages 공개 데모](./md/31_GitHub_Pages_공개데모.md). `demo/github-pages`를 수정해 `main`에 푸시하면 테스트 후 자동 배포됩니다.
